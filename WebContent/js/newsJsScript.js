...
function getNewsTemplate(seq, o) {
	var html = '';
	if(o.NEWS_TYPE == 2){
		html += '<div style="cursor: pointer;" class="div_td"  onclick="getDownloadFile(\''+o.FILE_ID+'\',\''+o.FILE_TYPE+'\')">';
	}else{
		html += '<div style="cursor: pointer;" class="div_td"  onclick="getNewsDetail(\''+o.NEWS_M_ID+'\')">';
	}
	
	html += '<span class="AOtime" >'+ o.CREATED_DATEStr +'</span>';
	html += '<h2 class="newsListTitle" >' + o.TOPIC + '</h2> ';
	if(o.NEWS_TYPE == 2){
		html += '<p>類別：'+o.NEWS_TYPE_NAME+'</p>';
	}else{
